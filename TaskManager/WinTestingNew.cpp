...
void SolverWidget::Solve( Solver& S, QPushButton* pBtn)
  {
  try
    {
    ExpStore::sm_pExpStore->Init_var();
    S.SetExpression( m_Expression );
...
void SolverWidget::SearchSolve()
  ...
  if( m_Expression.ConstExpr() || m_Expression.HasComplex() || m_Expression.HasMatrix() )
    ...
    if( IsType( TConstant, m_Expression ) )
      {
      Solve( TSin(), WinTesting::sm_pBottomWindow->GetCalcButton( 4, 0, 0 ) );
      Solve( TCos(), WinTesting::sm_pBottomWindow->GetCalcButton( 4, 0, 1 ) );
      Solve( TTan(), WinTesting::sm_pBottomWindow->GetCalcButton( 4, 0, 2 ) );
      Solve( TLg(), WinTesting::sm_pBottomWindow->GetCalcButton( 4, 1, 0 ) );
      Solve( TLn(), WinTesting::sm_pBottomWindow->GetCalcButton( 4, 1, 1 ) );
      Solve( TDegRad(), WinTesting::sm_pBottomWindow->GetCalcButton( 4, 1, 2 ) );
      Solve( TRadDeg(), WinTesting::sm_pBottomWindow->GetCalcButton( 4, 2, 0 ) );
      Solve( TSciCalc(), WinTesting::sm_pBottomWindow->GetCalcButton( 4, 2, 2 ) );
  ...
  auto SearchSystem = [&] ( const MathExpr& Ex )
    ...
    if( Sign != '=' )
      {
      Solve( SysInEqXY(), WinTesting::sm_pBottomWindow->GetCalcButton( 1, 1, 0 ) );
      Solve( SysInEq(), WinTesting::sm_pBottomWindow->GetCalcButton( 1, 0, 2 ) );
    ...
    if( TestNumerical && pNext->m_Memb.IsNumerical() )
      ...
      Solve( TSolvCalcEquation(), WinTesting::sm_pBottomWindow->GetCalcButton( 3, 2, 2 ) );
      return true;
      }
    if( !pNext->m_Memb.Binar( '=', Left, Right ) ) return true;
    Solve( MakeSubstitution(), WinTesting::sm_pBottomWindow->GetCalcButton( 1, 2, 0 ) );
    Solve( MakeExchange(), WinTesting::sm_pBottomWindow->GetCalcButton( 1, 2, 2 ) );
    Solve( SolveLinear(), WinTesting::sm_pBottomWindow->GetCalcButton( 1, 2, 1 ) );
    Prompt = X_Str( "MCanAlsoCalculate", "You can also calculate:" );
    return true;
    };
  ...
  if( m_Expression.Binar_( Sign, Left, Right ) )
    ...
    if( Sign != '=' )
      {
      Solve( RatInEq(), WinTesting::sm_pBottomWindow->GetCalcButton( 1, 1, 1 ) );
      Solve( SysInEq(), WinTesting::sm_pBottomWindow->GetCalcButton( 1, 0, 2 ) );
      Prompt = X_Str( "MCanAlsoCalculate", "You can also calculate:" );
      return Final(true);
      }
    Solve( TSolvDetLinEqu(), WinTesting::sm_pBottomWindow->GetCalcButton( 2, 0, 0 ) );
    if( m_pButtonBox->m_ButtonCount == 1 ) return Final( true );

    Prompt = X_Str( "MCanAlsoCalculate", "You can also calculate:" );
    Solve( TSolvDetQuaEqu(), WinTesting::sm_pBottomWindow->GetCalcButton( 2, 0, 2 ) );
    Solve( TSolvDisQuaEqu(), WinTesting::sm_pBottomWindow->GetCalcButton( 2, 1, 0 ) );
    Solve( TSolvDetVieEqu(), WinTesting::sm_pBottomWindow->GetCalcButton( 2, 1, 1 ) );
    Solve( TSolvQuaEqu(), WinTesting::sm_pBottomWindow->GetCalcButton( 2, 0, 1 ) );
    if( m_pButtonBox->m_ButtonCount > 0 ) return Final( true );

    Solve( TSolvCalcPolinomEqu(), WinTesting::sm_pBottomWindow->GetCalcButton( 2, 2, 2 ) );
    Solve( Log1Eq(), WinTesting::sm_pBottomWindow->GetCalcButton( 1, 0, 1 ) );
    Solve( ExpEq(), WinTesting::sm_pBottomWindow->GetCalcButton( 1, 1, 2 ) );
    Solve( TSolvCalcDetBiQuEqu(), WinTesting::sm_pBottomWindow->GetCalcButton( 2, 1, 2 ) );
    Solve( TSolvFractRatEq(), WinTesting::sm_pBottomWindow->GetCalcButton( 2, 2, 0 ) );
    Solve( TSolvCalcIrratEq(), WinTesting::sm_pBottomWindow->GetCalcButton( 2, 2, 1 ) );
    if( Formula.indexOf( "sin" ) != -1 )
      Solve( TSolvCalcSimpleTrigoEq(), WinTesting::sm_pBottomWindow->GetCalcButton( 3, 0, 0 ) );
    if( Formula.indexOf( "tan" ) != -1 )
      Solve( TSolvCalcSimpleTrigoEq(), WinTesting::sm_pBottomWindow->GetCalcButton( 3, 0, 1 ) );
    if( Formula.indexOf( "cos" ) != -1 )
      Solve( TSolvCalcSimpleTrigoEq(), WinTesting::sm_pBottomWindow->GetCalcButton( 3, 1, 0 ) );
    if( Formula.indexOf( "cot" ) != -1 )
      Solve( TSolvCalcSimpleTrigoEq(), WinTesting::sm_pBottomWindow->GetCalcButton( 3, 1, 1 ) );
    Solve( TSolvCalcSinCosEqu(), WinTesting::sm_pBottomWindow->GetCalcButton( 3, 0, 2 ) );
    Solve( TSolvCalcTrigoEqu(), WinTesting::sm_pBottomWindow->GetCalcButton( 3, 1, 2 ) );
    Solve( TSolvFractRatEq(), WinTesting::sm_pBottomWindow->GetCalcButton( 3, 2, 0 ) );
    Solve( TSolvCalcHomogenTrigoEqu(), WinTesting::sm_pBottomWindow->GetCalcButton( 3, 2, 1 ) );
    return Final(true);
    }
  Prompt = X_Str( "MCanAlsoCalculate", "You can also calculate:" );
  ...
  if( PosPow != -1 && Formula.indexOf( '^', PosPow + 1 ) != -1 )
    ...
    if( iPos2 != -1 && Formula.indexOf( '2', iPos2 + 1 ) != -1 )
      {
      Solve( TSolvSumCub(), WinTesting::sm_pBottomWindow->GetCalcButton( 0, 1, 1 ) );
      Solve( TSolvSubCub(), WinTesting::sm_pBottomWindow->GetCalcButton( 0, 2, 1 ) );
      Solve( TSolvSqrSubSum(), WinTesting::sm_pBottomWindow->GetCalcButton( 0, 2, 0 ) );
      Solve( TSolvSqrSubSum(), WinTesting::sm_pBottomWindow->GetCalcButton( 0, 1, 0 ) );
      Solve( TSolvSubSqr(), WinTesting::sm_pBottomWindow->GetCalcButton( 0, 0, 1 ) );
      if( m_pButtonBox->m_ButtonCount > 0 ) return Final( true );
      }
    }
  Solve( TSolvSumCub(), WinTesting::sm_pBottomWindow->GetCalcButton( 0, 1, 1 ) );
  ...
  Solve( TSolvSubCub(), WinTesting::sm_pBottomWindow->GetCalcButton( 0, 2, 1 ) );
  if( m_pButtonBox->m_ButtonCount > 0 ) return Final( true );
  Solve( TSolvReToMult(), WinTesting::sm_pBottomWindow->GetCalcButton( 0, 0, 0 ) );
  Solve( TSolvExpand(), WinTesting::sm_pBottomWindow->GetCalcButton( 0, 0, 2 ) );
  Solve( TTrinom(), WinTesting::sm_pBottomWindow->GetCalcButton( 0, 1, 2 ) );
  if( m_pButtonBox->m_ButtonCount > 0 ) return Final( true );
  Solve( Alg1Calculator(), WinTesting::sm_pBottomWindow->GetCalcButton( 0, 2, 2 ) );
  Final(true);
  }
...
WinTesting::WinTesting() : m_Review(false)
  ...
  m_pFile->addSeparator();
  m_pQuit = m_pFile->addAction( "&Quit", qApp, SLOT( quit() ), QKeySequence( "CTRL+Q" ) );
  ...
  sm_pAddPicture = sm_pEditor->addAction( "Add &Picture", this, SLOT( AddPicture() ) );
...
bool WinTesting::event( QEvent *event )
  ...
  if( event->type() == QEvent::Close )
    ...
    if( sm_pSaveTaskFile->isEnabled() && QMessageBox::question( nullptr, "Task Not Saved",
      "Task " + s_Task.m_pFile->fileName() + " was not saved, save?" ) == QMessageBox::Yes )
      {
      slotSaveTaskFile();
      event->ignore();
      return false;
...
void WinTesting::AddPicture()
  {
  QString Path = QFileDialog::getOpenFileName( nullptr, "Selection of picture" );
  if( Path.isEmpty() ) return;
  QString Html = "<img src=\"" + Path.mid( Path.lastIndexOf( '\\' ) + 1 ) + "\" style=\"vertical-align:middle;\" />";
...
void TaskHeaderEditor::accept()
  {
  m_TaskHeader.m_ShowUnarMinus = m_pShowUnarMinus->isChecked();
  m_TaskHeader.m_HideUnarMinus = m_pHideUnarMinus->isChecked();
  m_TaskHeader.m_ShowMinusByAddition = m_pShowMinusByAddition->isChecked();
  m_TaskHeader.m_HideMinusByAddition = m_pHideMinusByAddition->isChecked();
  m_TaskHeader.m_ShowMultSign = m_pShowMultSign->isChecked();
  ...
  m_TaskHeader.m_Accuracy = m_pAccuracy->value();
...
RowEditor::RowEditor( int RowNumber, TaskCalcEditor *pReceiver ) : m_RowNumber( RowNumber ), m_pReceiver( pReceiver )
  {
  setAcceptDrops( true );
  }

QString RowEditor::ToUnicode(const QByteArray& B)
  {
  QString Formula;
...
void RowEditor::EditCase()
  {
  CaseEditor CE( this, TaskCalcEditor::sm_Calc[m_RowNumber].m_Variable, text().toLocal8Bit().mid( 6 ) );
  CE.show();
...
void RowEditor::mouseDoubleClickEvent( QMouseEvent *event )
  ...
  s_iDogOption = 1;
  MathExpr Expr = MathExpr( Parser::StrToExpr( XPGedit::ExpandFuncName(FromUnicode(text() ) ) ) );
  s_iDogOption = 0;
  if( s_GlobalInvalid || Expr.IsEmpty() ) return;
...
CalcRow::CalcRow( QGridLayout *pVBox, TaskCalcEditor* pEditor, int iRow ) : 
  ...
  m_pVariable->setText( m_Pair.m_Variable );
...
void CalcRow::SaveText()
  ...
  m_Pair.m_Variable = m_pVariable->text().toLocal8Bit().trimmed();
...
m_pGrid(new QGridLayout), m_LastFocused(-1)
  {
  setStyleSheet( "QWidget {font-size:12pt;}" );
...
void TaskCalcEditor::SetLastFocused( int LastFocused )
  {
  if( LastFocused == -1 ) return;
  m_Rows[LastFocused].m_pVariable->setFocus();
  }
...
void Equations2::HomogenEq()
  {
  Solve( TSolvCalcHomogenTrigoEqu() );
  }

void Equations2::Calculator()
  {
  Solve( TSolvCalcEquation() );
  }
...
void CalcWidget::Solve( Solver *pSolver )
  ...
  if( TestMode )
    {
    sm_Result << Solver.Name() << " " << Formula << " ";
    if( sm_Test.device()->isOpen() )
      {
      sm_Test << Solver.Code() << " " << Formula << "\r\n";
      sm_Test.flush();
      }
    }
  try
    ...
    ExpStore::sm_pExpStore->Init_var();
    TSolutionChain::sm_SolutionChain.Clear();
    Solver.SetExpression( Formula );
    }
  catch( ErrParser& Err )
    {
    MathExpr Result = Solver.Result();
  ...
  if( TestMode )
    {
    sm_Result << Solver.Result().WriteE() << " " << Comm << " " << s_XPStatus.GetCurrentMessage() << "\r\n";
    sm_Result.flush();
    }
  return Final();
  }
...
void CalcWidget::AutoTest()
  ...
  while( !sm_Test.atEnd() )
    ...
    sm_Result << S.Name() << " " << Formula << " ";
    try
      {
      ExpStore::sm_pExpStore->Init_var();
      S.SetExpression( Formula );
      }
    catch( ErrParser& Err )
      {
      sm_Result << Err.Message() << "\r\n";
    ...
    if( S.Success() )
      ...
      Comm = s_LastError;
    sm_Result << S.Result().WriteE() << " " << Comm << " " << s_XPStatus.GetCurrentMessage() << "\r\n";
    sm_Result.flush();
    }
  CloseFiles();
