...
//        AllowedChar = true;
    ...
    if( AllowedChar )
      {
      if( m_pL->m_pSub_L->m_pMother != nullptr &&  TypeConvert( EdTable ) != nullptr &&
        TypeConvert( EdTable )->m_GridState != TGRVisible )
        {
        EdTable& elementT = *TypeConvert( EdTable );
        if( elementT.m_GridState == TGRUnvisible && m_pL->m_pSub_L->m_pFirst != nullptr )
          elementT.InsertColumn();
...
//            elementT.MoveToNext( m_pL->m_pSub_L ) );
        ...
        {
/*
        if (m_pL->m_pSub_L->m_pFirst == nullptr && IsHebChar(Uact.c()))
          {
          EditString Dlg(ToLang(QByteArray(1, Uact.c())), true);
  ...
  if( Uact.act == actCtrlKey )
    ...
    if( Uact == "vk_Right" )
      ...
      if( m_pCurrentTable == nullptr || m_pCurrentTable->m_GridState != TGRUnvisible ||
          !m_pCurrentTable->MoveToNext( m_pL->m_pSub_L ) )
        {
...
EdList* EdTable::GetCell()
  {
  EdList* pResult = m_Body[m_Row][m_Col];
  if( ++m_Col < m_ColCount ) return pResult;
  m_Col = 0;
